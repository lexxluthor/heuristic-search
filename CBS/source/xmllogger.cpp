...
bool XmlLogger::getLog(const char *FileName, const std::string *LogParams)
{
    ...
    if (LogParams[CN_LP_PATH] == "" && LogParams[CN_LP_NAME] == "") {
        ...
        if (*(--LogParams[CN_LP_PATH].end()) != CHAR_PATH_SEPARATOR) 
            LogFileName.append(STR_PATH_SEPARATOR);
        std::string lfn;
        lfn.append(FileName);
        size_t found = lfn.find_last_of(STR_PATH_SEPARATOR);
        ...
        LogFileName.append(LogParams[CN_LP_PATH]);
        if (*(--LogParams[CN_LP_PATH].end()) != CHAR_PATH_SEPARATOR) LogFileName.append(STR_PATH_SEPARATOR);
        LogFileName.append(LogParams[CN_LP_NAME]);
    }
...
void XmlLogger::writeToLogAgentsPaths(const AgentSet& agentSet,
    ...
    summaryElement->SetAttribute(CNS_TAG_ATTR_SIC, SIC);
...
void XmlLogger::writeToLogAggregatedResults(std::map<int, int>& successCount,
    ...
    for (auto pair : successCount) {
        ...
        for (int i = 0; i < res.data[CNS_TAG_ATTR_SIC][pair.first].size(); ++i) {
