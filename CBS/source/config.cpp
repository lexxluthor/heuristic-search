...
bool Config::getConfig(const char *FileName)
{
    ...
    tinyxml2::XMLElement *root, *algorithm, *element, *options;
    ...
    for (tinyxml2::XMLElement* child = options->FirstChildElement(); child != nullptr; child = child->NextSiblingElement())
    {
    ...
    if (!element) {
        ...
        std::cout << "Log filename: " << element->GetText() << std::endl;
        LogParams[CN_LP_NAME] = element->GetText();
    }

    bool success = getText(options, CNS_TAG_AGENTS_FILE, agentsFile);
...
bool Config::getValueFromText(tinyxml2::XMLElement *elem, const char *name, const char *typeName, void *field) {
    ...
    if (!child) {
        std::cout << "Warning! No '" << name << "' tag found in XML file! Using default value." << std::endl;
        return false;
    }
    ...
    if (res != tinyxml2::XML_SUCCESS) {
        std::cout << "Warning! Couldn't get value from '" << name << "' tag! Using default value." << std::endl;
        return false;
    }
    return true;
}
...
bool Config::getValueFromAttribute(tinyxml2::XMLElement *elem, const char *elemName, const char *attrName, const char *typeName, void *field) {
    ...
    if (res != tinyxml2::XML_SUCCESS) {
        std::cout << "Warning! Couldn't get value from '" << attrName << "' attribute in '" <<
                    elemName << "' tag! Using default value." << std::endl;
        return false;
    }
    return true;
}
